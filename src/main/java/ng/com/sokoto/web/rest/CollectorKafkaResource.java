...
import ng.com.sokoto.config.KafkaSseConsumer;
import ng.com.sokoto.config.KafkaSseProducer;
...
@RestController
@RequestMapping("/api/collector-kafka")
public class CollectorKafkaResource {

    private final Logger log = LoggerFactory.getLogger(CollectorKafkaResource.class);

    private final MessageChannel output;
    private Sinks.Many<Message<String>> sink = Sinks.many().unicast().onBackpressureBuffer();

    public CollectorKafkaResource(@Qualifier(KafkaSseProducer.CHANNELNAME) MessageChannel output) {
        this.output = output;
    }
